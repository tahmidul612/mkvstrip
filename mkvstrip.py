...
class CustomParser(ConfigParser.ConfigParser):
    ...
    def safegetlist(self, section, option, default=[]):
        try:
            return [item.strip() for item in ConfigParser.ConfigParser.get(self, section, option).split(",")]
        except:
            return default
...
group.add_argument("-l", "--log", default=config.safeget("global", "log"),
                   const=config.safeget("global", "log", os.path.join(currentDir, "mkvstrip.log")), nargs="?",
                   action="store", help="Log to file in addition to STDOUT and STDERR.", dest="log")
...
group.add_argument("-g", "--debug", default=config.safegetboolean("global", "debug", False), action="store_true",
...
group.add_argument("-y", "--dry-run", default=config.safegetboolean("global", "dry_run", False), action="store_true",
                   help="Enable mkvmerge dry run for testing", dest="dry_run")
...
group.add_argument("-p", "--preserve-timestamp", default=config.safegetboolean("global", "preserve_timestamp", True),
                   action="store_true", help="Preserve timestamp of mkv file", dest="preserve_timestamp")
...
group.add_argument("-m", "--log-subtitle", default=config.safegetboolean("global", "log_subtitle", False),
                   action="store_true", help="Log if file doesn\'t have a subtitle track.", dest="log_subtitle")
...
group.add_argument("-c", "--remove-commentary", default=config.safegetboolean("global", "remove_commentary", True),
                   action="store_true", help="Keep commentary audio and subtitle tracks.", dest="remove_commentary")
...
language.add_argument("-a", "--audio-language", default=config.safegetlist("global", "audio_language", ["eng", "und"]),
                      action="append", required=not config.has_option("global", "audio_language"),
                      help="Audio languages to retain. May be specified multiple times.", dest="audio_language")
language.add_argument("-s", "--subtitle-language",
                      default=config.safegetlist("global", "subtitle_language", ["eng", "und"]), action="append",
                      help="Subtitle languages to retain. May be specified multiple times.", dest="subtitle_language")

# Add arguments for binary locations
binarys = parser.add_argument_group("binarys")
binarys.add_argument("-b", "--mkvmerge-bin", default=config.safeget("global", "mkvmerge"), action="store",
                     required=not config.has_option("global", "mkvmerge"), help="Path to mkvmerge binary.",
                     dest="mkvmerge_bin")
binarys.add_argument("-i", "--mkvinfo-bin", default=config.safeget("global", "mkvinfo"), action="store",
                     required=not config.has_option("global", "mkvinfo"), help="Path to mkvinfo binary.",
                     dest="mkvinfo_bin")
binarys.add_argument("-t", "--mkvpropedit-bin", default=config.safeget("global", "mkvpropedit"), action="store",
                     required=not config.has_option("global", "mkvpropedit"), help="Path to mkvpropedit binary.",
                     dest="mkvpropedit_bin")
...
for section in (item for item in config.sections() if not item == "global"):
    if not config.has_option(section, "path"):
        ...
        if config.has_option(section, "rename_movie") and config.safegetboolean(section, "rename_movie") is True:
            pathList.append((config.safeget(section, "path").decode(systemEncoding), u"movie"))
        elif config.has_option(section, "rename_tv") and config.safegetboolean(section, "rename_tv") is True:
            pathList.append((config.safeget(section, "path").decode(systemEncoding), u"tv"))
        else:
            pathList.append((config.safeget(section, "path").decode(systemEncoding), False))
...
group.add_argument("-r", "--rename-tv", default=None, action="store_true",
                   help="Rename video track names to include immediate parent directory.", dest="rename_tv")
group.add_argument("-e", "--rename-movie", default=None, action="store_true",
                   help="Use the filename to rename the video track names.", dest="rename_movie")
...
if args.log:
    if os.path.exists(args.log.encode(systemEncoding)):
        fileHandler = logging.handlers.TimedRotatingFileHandler(args.log.encode(systemEncoding), when="h", interval=1,
                                                                backupCount=4)
...
for key, value in vars(args).iteritems():
    ...
    if isinstance(value, unicode):
        logger.info("%s = %s", key.upper(), value.encode(systemEncoding))
    else:
...
class Track(object):
    ...
    def __str__(self):
        return (
        u"Track #%s (%s): %s - %s" % (self._id, self._language, self._codec, self._name if self._name else "")).encode(
            "UTF-8")

    def __makeUnicode(self, data):
        if data is None or isinstance(data, unicode):
            return data
        else:
    ...
    @id.setter
    def id(self, value):
        self._id = self.__makeUnicode(value)
    ...
    @name.setter
    def name(self, value):
        self._name = self.__makeUnicode(value)
    ...
    def codec(self, value):
        self._codec = self.__makeUnicode(value)
    ...
    def language(self, value):
        self._language = self.__makeUnicode(value)
...
def checkTracks(element):
    ...
    for Atrack in element.find("Segment").find("Segment-Tracks").findall("A-Track"):
        ...
        try:
            ...
            if language:
                trackInfo.language = language.lower()
            else:
                trackInfo.language = "und"
        except:
            logger.debug("No Language was found for track %s", Atrack)

        # Fetch Track Name
        try:
            trackInfo.name = Atrack.find("Name").text.split("_", 1)[-1]
        except:
            logger.debug("No name was found for track %s", Atrack)

        # Fetch Track Codec
        try:
            trackInfo.codec = Atrack.find("Codec-Id").text.split("_", 1)[-1]
        except:
            logger.debug("No Codec-Id was found for track %s", Atrack)

        # Fetch Track ID Number
        try:
            trackInfo.id = Atrack.find("Track-Number").text.replace(")", "").split(" ")[-1].strip()
        except:
            logger.debug("No Track-Number was found for track %s", Atrack)
...
def checkTitle(element):
    try:
        title = element.find("Segment").find("Segment-Information").find("Title").text
    except:
        return None
    else:
        if title is None or isinstance(title, unicode):
            return title
        else:
...
for path, rename in pathList:
    if os.path.isfile(path.encode(systemEncoding)):
        processList.append((path, rename))
    else:
...
for count, pathData in enumerate(processList, start=1):
    ...
    command = [args.mkvinfo_bin.encode(systemEncoding), "--command-line-charset", systemEncoding, "--output-charset",
               "UTF-8", path.encode(systemEncoding)]
    try:
        rootElement = mkvToXML(subprocess.check_output(command))
    ...
    if rename:
        ...
        if title != name:
            modifyCMD = [args.mkvpropedit_bin.encode(systemEncoding), path.encode(systemEncoding),
                         "--command-line-charset", systemEncoding, "--set", "title=%s" % name.encode(systemEncoding)]
            logger.info("Renaming title of mkv to %s" % name.encode(systemEncoding))
            if args.dry_run is True:
                logger.info("Renaming are not being applied because you are in dry run mode")
            else:
                try:
                    subprocess.check_output(modifyCMD)
                except subprocess.CalledProcessError as e:
    ...
    if "audio" in trackData:
        ...
        for track in trackData["audio"]:
            ...
            elif track.language.encode("ascii") in args.audio_language and (
                    args.remove_commentary is False or not u"commentary" in unicode(track.name).lower()):
        ...
        # Skip files that don't have the specified language audio tracks
            ...
            logger.error("No audio tracks matching specified language(s) for %s... Skipping.",
                         path.encode(systemEncoding))
            continue
        ...
        # Audio Tracks found, Log each track
            ...
            if len(unWantedAudio) == 0:
                logger.info("Removing audio languages(s): None")
            else:
                remuxRequired = True
    ...
    if "subtitles" in trackData:
        ...
        for track in trackData["subtitles"]:
            ...
            elif track.language.encode("ascii") in args.subtitle_language and (
                    args.remove_commentary is False or not u"commentary" in unicode(track.name).lower()):
        ...
        # Skip files that don't have the specified language subtitle tracks
            ...
            if args.log_subtitle: logger.warning("No subtitle tracks matching specified language(s) for %s",
                                                 path.encode(systemEncoding))
        ...
        # Subtitle Tracks found, Log each track
            ...
            if len(unWantedSubtitle) == 0:
                logger.info("Removing subtitle languages(s): None")
            else:
                remuxRequired = True
    ...
    if len(unWantedAudio) > 0:
        ...
        for count, track in enumerate(wantedAudio): buildCMD.extend(
            ["--default-track", ":".join((track.id.encode("ascii"), "0" if count else "1"))])
    ...
    if len(unWantedSubtitle) > 0:
        ...
        for count, track in enumerate(wantedSubtitle): buildCMD.extend(
            ["--default-track", ":".join((track.id.encode("ascii"), "0"))])
    ...
    if args.dry_run is True:
        logger.info("Remux is not being applied because you are in dry run mode")
    else:
        try:
            ...
            sys.stdout.write("\n")
            if retcode:
                raise subprocess.CalledProcessError(retcode, buildCMD, output=process.communicate()[0])
            else:
                logger.info("Remux of %s successful", path.encode(systemEncoding))
            totalProcessed += 1
    ...
    if args.preserve_timestamp is True:
        ...
        if args.dry_run is True:
            logger.info("Preserving timestamp is not being applied because you are in dry run mode")
        else:
